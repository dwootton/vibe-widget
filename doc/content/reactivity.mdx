...
## Events from widgets (JS → Python)

There is no separate "JS → Python action" primitive. Widgets report events by updating an output,
and Python reacts by observing it.

```javascript
// In your widget JS
function handleResetClick() {
  model.set("last_event", { type: "reset_clicked", ts: Date.now() });
  model.save_changes();
}
```
...
```python
def on_event(change):
    if change.new["type"] == "reset_clicked":
        print("User clicked reset!")

widget.outputs.last_event.observe(on_event)
```

The timestamp ensures the observer fires even if the same event happens twice in a row.
...
When you select points in the scatter plot, the histogram updates via trait syncing. Outputs are
