...
export const ModeSwitch = ({ React, mode = "bike", onChangeMode }) => {
      ...
      {modes.map((m, idx) => {
        ...
        return (
          <React.Fragment key={m}>
            {idx > 0 && <span style={{ color: "#d9d9d9", userSelect: "none" }}>·</span>}
            <button
              type="button"
...
export const VerdictLines = ({
  ...
  const minBack = targetShop[`${mode}_back`] ?? minThere;
  const dwellMin = 10;
  const arriveMin = (leaveMin + minThere) % 720;
  const backMin = leaveMin + minThere + dwellMin + minBack;
  const backHhmm = formatHHMM(backMin);

  const openStatus = checkShopOpen(targetShop.hours, targetDay, arriveMin);
  let openWord = "open";
  if (openStatus === false) openWord = "closed";
  else if (openStatus === null) openWord = "hours unlisted";

  const shopLabel = (targetShop.name || "target").toLowerCase();
  const line1 = `leave ${formatHHMM(leaveMin)} → ${shopLabel} ${formatHHMM(arriveMin)} · ${openWord} → 10 min → back ${backHhmm}`;

  let availableDuration = backByMin - leaveMin;
  if (availableDuration < 0) availableDuration += 720;
  const totalTripMin = minThere + dwellMin + minBack;
  const diff = backByMin - (leaveMin + totalTripMin);

  let line2Text = "";
  let line2Color = "#2b8a3e";

  if (diff >= 0) {
    line2Text = `${diff} min before ${formatHHMM(backByMin)}`;
    line2Color = "#2b8a3e";
  } else {
    line2Text = `${Math.abs(diff)} min after ${formatHHMM(backByMin)}`;
    line2Color = "#c92a2a";
  }
  ...
  return (
    ...
    >
      <div style={{ color: "#111111" }}>{line1}</div>
      <div style={{ color: line2Color, fontWeight: 600 }}>{line2Text}</div>
    </div>
  );
};
...
export const HoverCard = ({ shop, leaveMin, targetDay, mode = "bike" }) => {
  ...
  const minThere = shop[`${mode}_min`] ?? 0;
  const arrivalMin = (leaveMin + minThere) % 720;
...
export const MapContainer = ({
  ...
  React.useEffect(() => {
    ...
    if (!routes || !routes[mode] || targetIndex === null || targetIndex === undefined) return;
    const routeCoords = routes[mode][targetIndex];
    ...
    const targetShop = data && data[targetIndex];
    const nMin = targetShop ? Math.round(targetShop[`${mode}_min`] ?? 0) : 0;

    const midLabel = L.marker(midPt, {
      icon: L.divIcon({
    ...
    routeLabelRef.current = midLabel;
  ...
  React.useEffect(() => {
    ...
    const markers = (data || []).map((shop, idx) => {
      ...
      const thereMin = shop[minCol] ?? 0;
      const backMin = shop[backCol] ?? thereMin;
      const roundTripMin = thereMin + 10 + backMin;
      const fitsBeforeBackBy = roundTripMin <= availableDuration;

      const arrivalMin = (leaveMin + thereMin) % 720;
      const arrivalStr = formatHHMM(arrivalMin);
...
export const TimeDial = ({
  ...
  const tripParts = React.useMemo(() => {
    ...
    const minThere = targetShop[`${mode}_min`] ?? 0;
    const minBack = targetShop[`${mode}_back`] ?? minThere;
    const totalTripMin = minThere + 10 + minBack;

    const arriveShopMin = leaveMin + minThere;
    ...
    const backAngle = minToAngle(returnMin) - Math.PI / 2;
    const backTickInner = {
      x: cx + (rRim - 5) * Math.cos(backAngle),
      y: cy + (rRim - 5) * Math.sin(backAngle),
    };
    const backTickOuter = {
      x: cx + (rRim + 5) * Math.cos(backAngle),
      y: cy + (rRim + 5) * Math.sin(backAngle),
  ...
  }, [leaveMin, backByMin, targetShop, mode, cx, cy, rTrip, rRim]);
  ...
  return (
    ...
    >
      <ModeSwitch React={React} mode={mode} onChangeMode={onChangeMode} />
      ...
      >
        ...
        {tripParts && (
          <g>
            ...
            <line
              x1={tripParts.backTickInner.x}
              y1={tripParts.backTickInner.y}
              x2={tripParts.backTickOuter.x}
              y2={tripParts.backTickOuter.y}
              stroke="#111111"
              strokeWidth="1.5"
              strokeDasharray="2, 2"
            />
...
export default function Widget({ model, React }) {
    ...
    React.useMemo(() => {
      ...
      (data || []).forEach((row, idx) => {
        ...
        if (dKm <= rad) {
          ...
          const thereMin = row[minCol] ?? 0;
          const returnMin = row[backCol] ?? thereMin;
          const roundTripMin = thereMin + 10 + returnMin;
          const arrivalMin = (leaveMin + thereMin) % 720;
  ...
  const { backHhmm, makesIt } = React.useMemo(() => {
    ...
    const thereMin = targetShop[`${mode}_min`] ?? 0;
    const returnMin = targetShop[`${mode}_back`] ?? thereMin;
    const totalTrip = thereMin + 10 + returnMin;
    const backMin = leaveMin + totalTrip;
    const hhmm = formatHHMM(backMin);

    let availableDuration = backByMin - leaveMin;
    if (availableDuration < 0) availableDuration += 720;
    const fits = totalTrip <= availableDuration;

    return { backHhmm: hhmm, makesIt: fits };
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <HoverCard shop={hoveredShop} leaveMin={leaveMin} targetDay={targetDay} mode={mode} />
