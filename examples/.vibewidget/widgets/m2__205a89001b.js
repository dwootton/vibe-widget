...
export const ClockDial = ({
  ...
  const handlePointerMove = (e) => {
    ...
    // Angle to 12-hour minutes (0..720)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Arrival tick and label at target */}
        {arrivePt && (
        ...
        {/* BACK BY handle (accent, sits 14px outside rim) */}
        <g
          style={{ cursor: "grab" }}
          onPointerDown={() => {
      ...
      {/* Day of week row: starts on Tuesday */}
      <div
        style={{
          display: "flex",
          gap: 3,
...
export default function Widget({ model, React }) {
  ...
  const [leaveMin, setLeaveMin] = React.useState(390); // 06:30 = 6*60 + 30
  const [backMin, setBackMin] = React.useState(525); // 08:45 = 8*60 + 45
  const [dayOfWeek, setDayOfWeek] = React.useState(1); // 1 = Tuesday
  ...
  React.useEffect(() => {
    ...
    // Bike reach bands: 30, 20, 10 minutes at 6%, 9%, 12% opacity
    const reachLayers = [];
    ...
    // Visual boundary circle (stays on top of reach)
    ...
    const shopRecords = data.map((shop, index) => {
      ...
      // Label marker for hh:mm arrival
    ...
    const handlePointerMove = (e) => {
      ...
      // Trigger update marks via state / helper
      renderShops(rKm);
    };
  ...
  const renderShops = React.useCallback(
    (currentRadiusKm) => {
      ...
      shopRecords.forEach(({ marker, labelMarker, shop, distKm, index }) => {
        ...
        // Open status: true (open), false (closed), null (unknown)
