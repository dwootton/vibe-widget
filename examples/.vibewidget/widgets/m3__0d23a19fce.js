...
function simplifyPoints(pts, tolSq = 16) {
  if (pts.length <= 3) return pts;
  const res = [pts[0]];
  let last = pts[0];
  for (let i = 1; i < pts.length; i++) {
    const d2 =
      (pts[i].x - last.x) * (pts[i].x - last.x) +
      (pts[i].y - last.y) * (pts[i].y - last.y);
    if (d2 >= tolSq || i === pts.length - 1) {
      res.push(pts[i]);
      last = pts[i];
    }
  }
...
export const RegionTable = ({ React, regions, stats, onHoverRow }) => {
  ...
  return (
    ...
    >
      ...
      >
        <thead>
          <tr style={{ borderBottom: `1px solid ${HAIRLINE_COLOR}` }}>
            <th style={{ textAlign: "left", padding: "4px 8px", fontWeight: 600 }}>region</th>
            <th style={{ textAlign: "right", padding: "4px 8px", fontWeight: 600 }}>n shops</th>
            <th style={{ textAlign: "right", padding: "4px 8px", fontWeight: 600 }}>n independent</th>
            <th style={{ textAlign: "right", padding: "4px 8px", fontWeight: 600 }}>n open on arrival</th>
            <th style={{ textAlign: "right", padding: "4px 8px", fontWeight: 600 }}>earliest arrival</th>
          </tr>
        </thead>
        <tbody>
          {regions.map((reg) => {
            const s = stats[reg.name] || {
              total: 0,
              independent: 0,
              openOnArrival: 0,
              earliestArrival: "—",
            ...
            return (
              <tr
                key={reg.id}
                onMouseEnter={() => onHoverRow(reg.name)}
              ...
              >
                <td style={{ padding: "5px 8px", fontWeight: 600 }}>{reg.name}</td>
                <td style={{ textAlign: "right", padding: "5px 8px" }}>{s.total}</td>
                <td style={{ textAlign: "right", padding: "5px 8px" }}>{s.independent}</td>
                <td style={{ textAlign: "right", padding: "5px 8px" }}>{s.openOnArrival}</td>
                <td
                  style={{
                    textAlign: "right",
                    padding: "5px 8px",
...
export default function Widget({ model, React }) {
  ...
  const computeRegionsData = (currentRegions, curLeave, curDay) => {
    const shops = shopsRef.current;
    const regDict = {};
    const statsDict = {};

    currentRegions.forEach((reg) => {
      const ring = reg.latlngs.map((ll) => [ll.lat, ll.lng]);
      const matchedIndices = [];
      let indepCount = 0;
      let openArrivalCount = 0;
      ...
      shops.forEach((s, idx) => {
        if (pointInPolygon([s.lat, s.lon], ring)) {
          ...
          const bMin = s.bike_min != null ? s.bike_min : 0;
          const arrMin = curLeave + bMin;
          const st = isOpenAt(s.hours, curDay, arrMin);
          if (st === "open") openArrivalCount++;
          if (arrMin < minArrival) minArrival = arrMin;
        }
      });

      regDict[reg.name] = matchedIndices;
      statsDict[reg.name] = {
        total: matchedIndices.length,
        independent: indepCount,
        openOnArrival: openArrivalCount,
        earliestArrival:
          minArrival !== Infinity ? formatMinutesToHHMM(minArrival) : "—",
      };
    });

    return { regDict, statsDict };
  };

  const recomputeAndSyncRegions = (updatedRegions) => {
    const regs = updatedRegions !== undefined ? updatedRegions : regionsRef.current;
    const { regDict, statsDict } = computeRegionsData(
      regs,
      leaveMinsRef.current,
      dayIndexRef.current
    );
    setRegionStats(statsDict);
    model.set("regions", regDict);
    model.save_changes();
  };
  ...
  const applyAllChanges = (
    ...
    const l = newLeave !== undefined ? newLeave : leaveMinsRef.current;
    const b = newBack !== undefined ? newBack : backMinsRef.current;
    const d = newDay !== undefined ? newDay : dayIndexRef.current;
    const t = newTarget !== undefined ? newTarget : targetIndexRef.current;
  ...
  React.useEffect(() => {
    ...
    const vertexIcon = L.divIcon({
      className: "region-vertex-icon",
      html: `
        <div style="width:16px;height:16px;margin:-8px 0 0 -8px;display:flex;align-items:center;justify-content:center;cursor:grab;">
          <div style="width:7px;height:7px;border-radius:50%;background:${INK_COLOR};border:1.5px solid #ffffff;box-sizing:border-box;"></div>
        </div>
      `,
      iconSize: [0, 0],
    });

    const removeRegion = (regId) => {
      if (regionLayersRef.current[regId]) {
        const { polygon, markers } = regionLayersRef.current[regId];
        regionsGroup.removeLayer(polygon);
        markers.forEach((m) => regionsGroup.removeLayer(m));
        delete regionLayersRef.current[regId];
      }
      const updated = regionsRef.current.filter((r) => r.id !== regId);
      regionsRef.current = updated;
      setRegions([...updated]);
      recomputeAndSyncRegions(updated);
    };

    const buildRegionLayers = (reg) => {
      ...
      const markers = reg.latlngs.map((ll, vIdx) => {
        const vm = L.marker(ll, {
          icon: vertexIcon,
          draggable: true,
          zIndexOffset: 1500,
        }).addTo(regionsGroup);
        ...
        vm.on("drag", (e) => {
          ...
          const targetReg = regionsRef.current.find((r) => r.id === reg.id);
          if (targetReg) {
            targetReg.latlngs[vIdx] = newPos;
            polygon.setLatLngs(targetReg.latlngs);
          }
        });
      ...
      regionLayersRef.current[reg.id] = { polygon, markers };
    };
    ...
    const onContainerMouseDown = (e) => {
      ...
      lassoPolyline = L.polyline([map.containerPointToLatLng(pt)], {
        color: INK_COLOR,
        weight: 1.5,
        dashArray: "3, 3",
      }).addTo(map);
      ...
      const onWindowMouseMove = (ev) => {
        ...
        const curPt = { x: ev.clientX - r.left, y: ev.clientY - r.top };
        lassoPoints.push(curPt);

        if (lassoPolyline) {
          const simplified = simplifyPoints(lassoPoints, 9);
          const latlngs = simplified.map((p) => map.containerPointToLatLng(p));
      ...
      const onWindowMouseUp = () => {
        ...
        const simplified = simplifyPoints(lassoPoints, 25);
        if (simplified.length >= 3) {
          const latlngs = simplified.map((p) => map.containerPointToLatLng(p));
          const name = nextRegionLetterRef.current;
          nextRegionLetterRef.current = String.fromCharCode(
            nextRegionLetterRef.current.charCodeAt(0) + 1
          );
          ...
          buildRegionLayers(newRegion);
          recomputeAndSyncRegions(nextRegions);
        }
      };
  ...
  React.useEffect(() => {
    Object.keys(regionLayersRef.current).forEach((regId) => {
      const reg = regionsRef.current.find((r) => r.id === regId);
      const layerObj = regionLayersRef.current[regId];
      if (!layerObj || !reg) return;
  ...
  return (
    ...
    >
      ...
      <RegionTable
        ...
        regions={regions}
        stats={regionStats}
        onHoverRow={setHoveredRegionName}
      />
    </div>
  );
}