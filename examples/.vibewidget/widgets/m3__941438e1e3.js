...
export const RegionsTable = ({
  ...
  const stats = React.useMemo(() => {
    return regionsList.map((reg) => {
      ...
      const earliestArrival = earliestMin !== Infinity ? minToHhmm(earliestMin) : "—";
  ...
  }, [regionsList, data, leaveMin, dayIndex]);
                ...
                return (
                  <tr
                    ...
                    style={{
                      ...
                      background: isHovered ? "rgba(230, 81, 0, 0.12)" : "transparent",
                      cursor: "pointer",
                  ...
                  >
                    ...
                    <td style={{ padding: "6px 8px", fontWeight: 600 }}>{row.nShops}</td>
                    <td
                      style={{
                        padding: "6px 8px",
                        fontWeight: 600,
...
export default function Widget({ model, React }) {
  ...
  // Lasso / Regions State:
  // regionsList: [ { id, name, color, latLngs: [[lat, lon], ...], indices: [number, ...] } ]
  ...
  // Region Leaflet objects storage: id -> { polygon, handles: [marker, ...], labelMarker }
  ...
  // Compute shops enclosed in a polygon
  ...
  // Update regions trait to python
  ...
  React.useEffect(() => {
    ...
    // Disabling boxZoom ensures Shift+drag does not trigger leaflet's shift box zoom
    ...
    const onMapMouseDown = (e) => {
      ...
      if (lassoSvgOverlayRef.current) {
        ...
        const polyline = lassoSvgOverlayRef.current.querySelector(".lasso-polyline");
        if (polyline) {
    ...
    const onMapMouseMove = (e) => {
      ...
      if (lassoSvgOverlayRef.current) {
        const polyline = lassoSvgOverlayRef.current.querySelector(".lasso-polyline");
        if (polyline) {
          const ptsStr = lassoPointsRef.current.map((p) => `${p.x},${p.y}`).join(" ");
    ...
    const onMapMouseUp = () => {
      ...
      if (pts.length < 5) return; // Too short to form a polygon

      // Simplify points: sample evenly to reduce vertex handles count (~16 vertices)
      ...
      // Convert pixel points to Leaflet LatLng coordinates
      const latLngs = sampled.map((p) => {
        const ll = map.containerPointToLatLng([p.x, p.y]);
        return [ll.lat, ll.lng];
      });

      // Name region sequentially A, B, C...
      ...
      const id = "region_" + Date.now() + "_" + Math.random().toString(36).substr(2, 5);
  ...
  React.useEffect(() => {
    ...
    regionsList.forEach((reg) => {
      ...
      if (!layerObj) {
        ...
        // Double click inside polygon deletes region
        ...
        // Small draggable handle on every vertex
        ...
        // Region Name Badge placed at polygon center
  ...
  // Live restyling whenever target, leaveMin, dayIndex change
  React.useEffect(() => {
    const openArr = updateMarkersAndLabels(
      currentRadiusRef.current,
      targetIndex,
