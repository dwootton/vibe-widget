  ...
  return (
    ...
    >
      ...
      <div
        style={{
          fontSize: "10px",
          color: shop.hours ? "#292524" : "#78716c",
          borderTop: "1px dashed #d6d3d1",
          paddingTop: "4px",
          marginTop: "4px",
          lineHeight: "1.3",
          ...
          <tbody>
            {rows.map((r) => (
              ...
              >
                ...
                <td style={{ ...cellBase, color: r.nOpen > 0 ? "#15803d" : "#57534e", fontWeight: 700 }}>
                  {r.nOpen}
                </td>
...
export default function Widget({ model, React }) {
  ...
  // Regions: [{ id, name, color, ring: [[lat,lng],...] }]
  ...
  const regionRenderRef = React.useRef(new Map()); // id -> {poly, handles:[markers], label}
  ...
  React.useEffect(() => {
    ...
    const styleId = "leaflet-custom-styles-clock-lasso";
  ...
  // ---- region stats (derived; recomputed on regions/time/day/data change) ----
  ...
  React.useEffect(() => {
    ...
    const map = L.map(containerRef.current, {
      ...
      zoom: 12.8,
      zoomControl: false,
      boxZoom: false, // shift+drag reserved for lasso
    });
    mapRef.current = map;
    ...
    hitCircle.on("mousedown", (e) => {
      if (e.originalEvent && e.originalEvent.shiftKey) return; // let lasso win
      isDraggingRef.current = true;
      map.dragging.disable();
      const pill = document.getElementById("radius-pill");
      if (pill) pill.classList.add("active");
    ...
    // region layer group sits above isochrones, below markers
    ...
    data.forEach((shop, index) => {
      ...
      marker.on("mouseover", () => {
        ...
        setHoveredShopInfo({ shop, arrivalTimeStr: arrStr, status: st });
      });
    ...
    const finishLasso = () => {
      ...
      // tolerance depends on zoom: ~10 screen px
      ...
      // cap vertex handles to something manageable
      if (ring.length > 24) {
      ...
      const name = String.fromCharCode(65 + (idx % 26)) + (idx >= 26 ? String(Math.floor(idx / 26)) : "");
  ...
  React.useEffect(() => {
    ...
    regions.forEach((reg) => {
      ...
      if (!entry) {
        ...
        // keep geometry in sync (e.g. after state commit)
