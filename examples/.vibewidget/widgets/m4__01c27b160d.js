...
export const CountLine = ({ insideCount, independentCount, openAndBackCount, React }) => (
  <div
    style={{
      position: "absolute",
      top: 14,
...
export const RegionsTable = ({ regionsSummary, hoveredRegionName, onHoverRegion, React }) => {
...
export const VerdictLines = ({
  ...
  const minThere = targetShop[`${mode}_min`] ?? 0;
  const minBack = targetShop[`${mode}_back`] ?? 0;
  const arriveMin = leaveMin + minThere;
  const openStatus = parseOsmHours(targetShop.hours, day, arriveMin);
  const backTimeMin = arriveMin + 10 + minBack;

  const leaveHhmm = minToHhmm(leaveMin);
  const arriveHhmm = minToHhmm(arriveMin);
  const backHhmm = minToHhmm(backTimeMin);
  const backByHhmm = minToHhmm(backMin);

  const shopNameLower = (targetShop.name || "").toLowerCase();
  const diff = backTimeMin - backMin;
  const makesIt = diff <= 0;

  const line1 = `leave ${leaveHhmm} \u2192 ${shopNameLower} ${arriveHhmm} · ${openStatus} \u2192 10 min \u2192 back ${backHhmm}`;
  const line2 = makesIt
    ? `${Math.abs(diff)} min before ${backByHhmm}`
    : `${diff} min after ${backByHhmm}`;
  ...
  return (
    ...
    >
      <div style={{ color: INK_COLOR, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {line1}
      </div>
      <div style={{ color: makesIt ? OPEN_COLOR : CLOSED_COLOR, fontWeight: 500, marginTop: 2 }}>
        {line2}
      </div>
    </div>
  );
...
export const TimeDial = ({
  ...
  if (targetShop) {
    ...
    const minBack = targetShop[`${mode}_back`] ?? 0;
    const arriveMin = leaveMin + minThere;
    const returnMin = leaveMin + minThere + 10 + minBack;

    const startAng = minToAngle(leaveMin);
    const arriveAng = minToAngle(arriveMin);
    const returnAng = minToAngle(returnMin);
    ...
    // Hollow tick on rim at the back time
    const tickOuter = getPt(returnAng, dialRadius + 5);
    const tickInner = getPt(returnAng, dialRadius - 5);
    const tickCircle = getPt(returnAng, dialRadius);
    backRimTick = {
      outer: tickOuter,
      inner: tickInner,
      center: tickCircle,
      makesIt: returnMin <= backMin,
    };
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Hollow tick on the rim at back time */}
        {backRimTick && (
          <circle
            cx={backRimTick.center.x}
            cy={backRimTick.center.y}
            r={4}
            fill="#ffffff"
            stroke={backRimTick.makesIt ? OPEN_COLOR : CLOSED_COLOR}
            strokeWidth={1.5}
          />
        )}
...
export default function Widget({ model, React }) {
  ...
  const evaluateState = (rKm, lMin, bMin, dIdx, curMode) => {
    ...
    for (let i = 0; i < shops.length; i++) {
      ...
      if (isInside) {
        ...
        const tMin = s[minKey] ?? 0;
        const bTime = s[backKey] ?? 0;
        const arriveMin = lMin + tMin;
        const roundTripMin = lMin + tMin + 10 + bTime;
  ...
  const syncOutputs = (rKm, insideIndices, openIndices, lMin, bMin, dIdx, tIdx, regionsMap, curMode) => {
    const shops = shopsRef.current;
    const targetShop = shops[tIdx];
    let backHhmm = "—";
    let makesIt = false;

    if (targetShop) {
      const minThere = targetShop[`${curMode}_min`] ?? 0;
      const minBack = targetShop[`${curMode}_back`] ?? 0;
      const backTimeMin = lMin + minThere + 10 + minBack;
      backHhmm = minToHhmm(backTimeMin);
      makesIt = backTimeMin <= bMin;
    }
  ...
  const updateReachLayer = (targetMode) => {
    const map = mapRef.current;
    if (!map) return;
    if (reachLayerRef.current) {
      map.removeLayer(reachLayerRef.current);
      reachLayerRef.current = null;
    }

    const reachData = model.get("reach");
    const modeReach = reachData ? reachData[targetMode] : null;
    if (modeReach) {
      const reachLayer = L.geoJSON(modeReach, {
        style: (feature) => {
      ...
      reachLayer.bringToBack();
      reachLayerRef.current = reachLayer;
    }
  };

  const updateStreetRoute = (targetMode, tIdx) => {
    const map = mapRef.current;
    if (!map) return;
    ...
    if (routeLabelRef.current) {
      map.removeLayer(routeLabelRef.current);
      routeLabelRef.current = null;
    }

    const routesData = model.get("routes");
    const modeRoutes = routesData ? routesData[targetMode] : null;
    const shop = shopsRef.current[tIdx];

    if (modeRoutes && modeRoutes[tIdx] && modeRoutes[tIdx].length > 0) {
      const pts = modeRoutes[tIdx];
      ...
      const minVal = shop ? Math.round(shop[`${targetMode}_min`] ?? 0) : 0;
      ...
      const labelMarker = L.marker(midPt, { icon: labelIcon, interactive: false }).addTo(map);
      routeLabelRef.current = labelMarker;
    }
  };
  ...
  React.useEffect(() => {
    ...
    shops.forEach((shop, index) => {
      ...
      marker.on("click", () => {
        ...
        updateMarkers();
        updateStreetRoute(modeRef.current, index);
      });
  ...
  // Update when dial, day, target, regions, or mode changes
  React.useEffect(() => {
    updateMarkers();
    updateStreetRoute(mode, targetIndex);
    const ev = evaluateState(radiusKm, leaveMin, backMin, day, mode);
    setCounts({
      inside: ev.insideCount,
      independent: ev.independentCount,
      openAndBack: ev.openAndBackCount,
  ...
  }, [leaveMin, backMin, day, targetIndex, regions, mode]);
  ...
  return (
    ...
    >
      <style>{`
        ...
        .custom-shop-marker, .vertex-handle-icon, .region-label-icon, .route-mid-label {
