...
// Route itinerary component
export const RouteItinerary = ({ React, route, data, legs, leaveHours, leaveMinutes }) => {
  if (route.length === 0) {
    return (
      <div style={{ 
        padding: '12px', 
  ...
  const stops = route.map((shopIdx, i) => {
    ...
    // legs[0] is hotel, legs[i+1] is data row i
    // From previous location to this shop
    ...
    // Add stop time for next leg calculation
    if (i < route.length - 1) {
      cumulativeMin += STOP_TIME;
    }
  ...
  return (
    <div>
      <table style={{ 
        ...
        <thead>
          <tr style={{ borderBottom: '2px solid #ddd' }}>
            <th style={{ padding: '6px 8px', textAlign: 'center', color: '#333', fontWeight: 600, width: 30 }}>#</th>
            <th style={{ padding: '6px 8px', textAlign: 'left', color: '#333', fontWeight: 600 }}>Shop</th>
            <th style={{ padding: '6px 8px', textAlign: 'right', color: '#333', fontWeight: 600 }}>Leg</th>
            <th style={{ padding: '6px 8px', textAlign: 'right', color: '#333', fontWeight: 600 }}>Arrive</th>
          </tr>
        </thead>
        <tbody>
          {stops.map(stop => (
            <tr key={stop.idx} style={{ borderBottom: '1px solid #eee' }}>
              <td style={{ padding: '6px 8px', textAlign: 'center', fontWeight: 700, color: '#2563eb' }}>{stop.stopNum}</td>
              <td style={{ padding: '6px 8px', color: '#1a1a1a' }}>{stop.name}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: '#666', fontFamily: 'monospace' }}>
                {stop.legMin != null ? `${stop.legMin} min` : '—'}
              </td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: '#16a34a', fontWeight: 600, fontFamily: 'monospace' }}>
                {stop.arrivalTime}
              </td>
            </tr>
          ))}
...
export default function Widget({ model, React }) {
  ...
  const updateMarkers = React.useCallback((radius, day, leaveH, leaveM, currentRoute) => {
    const leaveTotalMin = leaveH * 60 + leaveM;
    ...
    markersRef.current.forEach(({ marker, label, shop, idx }) => {
      ...
      marker.setStyle({
        ...
        radius: isInside ? 8 : 5,
        color: strokeColor,
        weight: isPinned ? 3 : (isInside ? 2 : 1),
        dashArray: dashArray,
  ...
  // Update route polyline and pin markers
  const updateRouteLayer = React.useCallback((currentRoute) => {
    ...
    // Build route points starting from hotel
    ...
    currentRoute.forEach((shopIdx, i) => {
      ...
      pinMarker.bindTooltip(`<div style="font-weight:bold">${i + 1}. ${shop.name}</div><div style="color:#666;font-size:11px">Click to unpin</div>`, {
        direction: 'top',
        offset: [0, -12]
      });
  ...
  // Update route layer when route changes
  React.useEffect(() => {
    updateRouteLayer(route);
  }, [route, updateRouteLayer]);
  ...
  React.useEffect(() => {
    ...
    updateMarkers(radiusKm, selectedDay, leaveHours, leaveMinutes, route);
    updateLabel(radiusKm);
  }, [radiusKm, selectedDay, leaveHours, leaveMinutes, regions, route, updateAll, updateMarkers, updateLabel]);

  // Toggle pin for a shop
  const togglePin = React.useCallback((shopIdx) => {
    setRoute(prev => {
      const idx = prev.indexOf(shopIdx);
      if (idx >= 0) {
        // Unpin - remove from route
        return [...prev.slice(0, idx), ...prev.slice(idx + 1)];
      } else {
        // Pin - add to end of route
        return [...prev, shopIdx];
      }
    });
  }, []);
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ 
        ...
        <div style={{ fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 8 }}>
          Route Itinerary <span style={{ fontWeight: 400, color: '#666' }}>(click pinned shop to remove)</span>
        </div>
        <RouteItinerary 
          React={React}
          route={route}
