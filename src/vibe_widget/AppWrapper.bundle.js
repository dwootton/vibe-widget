...
import * as React11 from "https://esm.sh/react@18";
import { createRoot } from "https://esm.sh/react-dom@18/client";
import htm9 from "https://esm.sh/htm@3";
...
function ProgressMap({ logs, fullHeight = false }) {
...
function EditPromptPanel({ elementBounds, elementDescription, initialPrompt, onSubmit, onCancel }) {
  ...
  React6.useEffect(() => {
    ...
    let top2 = elementBounds.bottom + GAP;
    let left = elCenterX - PANEL_WIDTH / 2;
    if (top2 + panelHeight + PADDING > viewport.height) {
      top2 = elementBounds.top - panelHeight - GAP;
    }
    if (top2 < PADDING) {
      top2 = Math.max(PADDING, elCenterY - panelHeight / 2);
    ...
    top2 = Math.max(PADDING, Math.min(top2, viewport.height - panelHeight - PADDING));
    setPosition({ top: top2, left });
...
var html7 = htm7.bind(React7.createElement);
...
import htm8 from "https://esm.sh/htm@3";
...
function useModelSync(model) {
  const [status, setStatus] = React9.useState(model.get("status"));
...
import * as React10 from "https://esm.sh/react@18";
...
// src/vibe_widget/AppWrapper/AppWrapper.js
var html9 = htm9.bind(React11.createElement);
ensureGlobalStyles();
...
function AppWrapper({ model }) {
  ...
  const [showSource, setShowSource] = React11.useState(false);
  const [sourceError, setSourceError] = React11.useState("");
  const [renderCode, setRenderCode] = React11.useState(code || "");
  const [lastGoodCode, setLastGoodCode] = React11.useState(code || "");
  const [applyState, setApplyState] = React11.useState({
    pending: false,
    previousCode: "",
    nextCode: ""
  });
  const containerRef = React11.useRef(null);
  const [minHeight, setMinHeight] = React11.useState(0);
  const [hasAuditAck, setHasAuditAck] = React11.useState(() => {
    try {
  ...
  const [showAudit, setShowAudit] = React11.useState(false);
  ...
  return html9`
    ...
    transition: "opacity 0.3s ease"
      ...
      ${grabMode && grabMode !== "selecting" && html9`
        <${EditPromptPanel}
...
function render({ model, el }) {
  ...
  rootInstance.render(html9`<${AppWrapper} model=${model} />`);
}
